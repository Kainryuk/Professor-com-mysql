...
const Register = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (typeof register !== 'function') {
      console.error('❌ [REGISTER] Hook useAuth.register não está disponível');
      return;
    }
    ...
    // Limpar CPF para envio (remover pontos e traços) e incluir senha
    const dataToSend = {
    ...
    try {
      ...
      const result = (await register(dataToSend)) as unknown as RegisterResponse;

      if (result?.success) {
        console.log('✅ [REGISTER] Cadastro bem-sucedido! Redirecionando para login...');
        navigate('/login');
      } else {
        const errMsg = result?.error ?? 'Resposta inválida do servidor';
        console.error('❌ [REGISTER] Falha no cadastro:', errMsg);
      }
    } catch (err) {
      console.error('❌ [REGISTER] Erro ao chamar register:', err);
    } finally {
      setLoading(false);
    }
  };
